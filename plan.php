...
try {
    if (!isset($_GET['y'], $_GET['c'], $_GET['t'])) {
        ...
        if (count($html->find("span[class=molecule__label]")) === 0) {
            ...
            if ($start === null) {
                echo "there is no requirements or perquisites for this plan on ANU Programs and Courses.";
            } else {
    ...
    echo "There was an error, please try again later (Error: " . $e->getMessage() . ").";
